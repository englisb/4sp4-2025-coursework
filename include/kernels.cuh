...
// Matrix-Matrix multiplication kernel: C = A * B^T
// A is (m x k), B is (n x k), C is (m x n)
inline __global__ void MM(const float *A, const float *B, float *C, int m,
                          int n, int k) {
  __shared__ float As[TILE_SIZE][TILE_SIZE];
  __shared__ float Bs[TILE_SIZE][TILE_SIZE];
...
// result is (m x n)
inline __global__ void SpMM(int *row_ptr, int *col_id, const float *a,
                            const float *b, float *result, int m, int n,
                            int k) {
  int row = blockIdx.x * blockDim.x + threadIdx.x;
...
// result = A * b, where A is sparse (CSR format) (m x n), b is (n x 1), result
// is (m x 1)
inline __global__ void SpMV(int *row_ptr, int *col_id, const float *a,
                            const float *b, float *result, int m, int n) {
  int row = blockIdx.x * blockDim.x + threadIdx.x;
