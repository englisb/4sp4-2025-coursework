...
void nvbench_gemm(nvbench::state& state)
{
    ...
    state.exec(nvbench::exec_tag::timer, [&](nvbench::launch& launch, auto& timer){
        timer.start();
        swiftware::hpp::gemm_gpu_baseline<<<blocks, threads>>>(n, n, n, d_a, d_b, d_c);
...
void nvbench_spmm(nvbench::state& state)
{
    ...
    CUDA_CHECK(cudaMemcpy(d_col_id, col_id.data(), nnz * sizeof(int), cudaMemcpyHostToDevice));
    ...
    state.exec(nvbench::exec_tag::timer, [&](nvbench::launch& launch, auto& timer){
        timer.start();
        swiftware::hpp::SpMM<<<blocks, threads>>>(d_row_ptr, d_col_id, d_values, d_b, d_c, n, n, n);
        CUDA_CHECK(cudaDeviceSynchronize());
        timer.stop();
    });
...
void nvbench_spmv(nvbench::state& state)
{
    ...
    state.exec(nvbench::exec_tag::timer, [&](nvbench::launch& launch, auto& timer){
        timer.start();
        swiftware::hpp::SpMV<<<blocks, threads>>>(d_row_ptr, d_col_id, d_values, d_b, d_c, n, n);
        CUDA_CHECK(cudaDeviceSynchronize());
        timer.stop();
    });
...
NVBENCH_BENCH(nvbench_gemm).set_name("GEMM_Baseline").add_int64_axis("n", {256, 512, 1024, 2048, 4096});
NVBENCH_BENCH(nvbench_gemm_shared).set_name("GEMM_Shared").add_int64_axis("n", {256, 512, 1024, 2048, 4096});
NVBENCH_BENCH(nvbench_gemm_coalesced).set_name("GEMM_Coalesced").add_int64_axis("n", {256, 512, 1024, 2048, 4096});
NVBENCH_BENCH(nvbench_gemm_combined).set_name("GEMM_Combined").add_int64_axis("n", {256, 512, 1024, 2048, 4096});
NVBENCH_BENCH(nvbench_gemv).set_name("GEMV").add_int64_axis("n", {256, 512, 1024, 2048, 4096});
NVBENCH_BENCH(nvbench_spmm).set_name("SpMM").add_int64_axis("n", {256, 512, 1024, 2048, 4096}).add_float64_axis("sparsity", {50.0, 70.0, 90.0});
NVBENCH_BENCH(nvbench_spmv).set_name("SpMV").add_int64_axis("n", {256, 512, 1024, 2048, 4096}).add_float64_axis("sparsity", {50.0, 70.0, 90.0});
