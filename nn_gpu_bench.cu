...
void nvbench_gpu_dense_nn_gemm(nvbench::state& state)
{
    ...
    for (int i = 0; i < batchSize; i++) {
        for (int j = 0; j < features->n; j++) {
            batchFeatures->data[i * features->n + j] = features->data[i * features->n + j];
        }
    }

    state.exec([&](nvbench::launch&) {
        auto *predictions = swiftware::hpp::gpu_dense_nn_gemm(
            batchFeatures, weightsHidden, weightsOutput, biasesHidden, biasesOutput, scheduleParams);
        
        // Calculate accuracy
        int correctPredictions = 0;
        for (int i = 0; i < batchSize; i++) {
            if (static_cast<int>(predictions->data[i]) == static_cast<int>(labels->data[i])) {
                correctPredictions++;
            }
        }
        double accuracy = (static_cast<double>(correctPredictions) / batchSize) * 100.0;
...
void nvbench_gpu_sparse_nn_spmm(nvbench::state& state)
{
    ...
    state.exec([&](nvbench::launch&) {
        auto *predictions = swiftware::hpp::gpu_sparseNNSpmm(
            batchFeatures, weightsHiddenCSR, weightsOutputCSR, biasesHidden, biasesOutput, scheduleParams);
...
NVBENCH_BENCH(nvbench_gpu_dense_nn_gemm)
    ...
    .add_int64_axis("batch_size", {10, 60000});

NVBENCH_BENCH(nvbench_gpu_dense_nn_gemv)
    .set_name("GPU_Dense_NN_GEMV");
