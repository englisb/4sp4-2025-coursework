...
void gemm_simd(int m, int n, int k, const float *A, const float *B, float *C,
               ScheduleParams Sp) {
  // GEMM: C = A * B + C
  // A is m x k (row-major), B is k x n (row-major), C is m x n (row-major)
...
// SIMD + OpenMP parallel implementation
void gemm_simd_parallel(int m, int n, int k, const float *A, const float *B,
...
// Tiled + SIMD + OpenMP parallel implementation (most optimized)
void gemm_tiled_simd_parallel(int m, int n, int k, const float *A,
  ...
  int tile_m = (Sp.TileSize1 > 0) ? Sp.TileSize1 : 32;
  int tile_n = (Sp.TileSize2 > 0) ? Sp.TileSize2 : 32;
