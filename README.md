...
Question 1: Standard Sort Algorithm Complexity

![Figure 1: Sorting Algorithm Performance /(Median Real Time/)](plots/sorting_performance_median_realtime.png) shows the median real time in microseconds as a function of input size for four sorting algorithms. The results confirm the expected theoretical time complexities. Quick Sort and STD Sort follow O(n log n) growth, with their curves increasing gradually and remaining well below the quadratic algorithms across the full range of n. Selection Sort and Bubble Sort rise much more steeply, reflecting their O(n^2) behaviour.

At smaller input sizes, the runtimes for all algorithms appear close together because of constant overhead and timer resolution effects, which mask the true scaling. However, once the input size grows beyond a few hundred elements, the distinction becomes clear. Quick Sort and STD Sort maintain efficient scaling and continue to follow the n log n trend, while Selection and Bubble Sort diverge sharply. Bubble Sort in particular grows the fastest and quickly becomes computationally impractical. This behaviour demonstrates that the observed results align directly with theoretical complexity: n log n algorithms remain efficient at scale, whereas n^2 algorithms rapidly dominate runtime. 
Question 2: Difference Between Runs and Surrogates

![Figure 2: Median L1 Data-Cache Loads vs Array Size](plots/median_l1_dcache_loads.png) shows the median number of retired L1 data-cache loads for each algorithm as input size increases. This measure is a surrogate for work volume since higher load counts correspond to more memory traffic and more executed operations. The observed trends mirror the runtime results from Figure 1. Quick Sort and STD Sort scale approximately like n log n, producing steadily increasing but controlled cache load counts. In contrast, Selection Sort and Bubble Sort scale like n^2, generating orders of magnitude more cache loads as n increases.

This difference explains the divergence in runtime performance. Because the cost per cache load does not decrease with array size, the excessive number of loads in the quadratic algorithms directly drives their execution time to explode. On the other hand, Quick Sort and STD Sort keep cache loads bounded, which ensures that their runtime grows much more slowly and remains consistent with n log n scaling.
