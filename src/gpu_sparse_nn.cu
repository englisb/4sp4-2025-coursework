...
DenseMatrix *gpu_sparseNNSpmm(DenseMatrix *InData, CSR *W1, CSR *W2, 
    ...
    CUDA_CHECK(cudaMemcpy(d_W1_row_ptr, W1->Ap.data(), (W1->m + 1) * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_W1_col_id, W1->Ai.data(), W1->nnz * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_W1_val, W1->Ax.data(), W1->nnz * sizeof(float), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(d_W2_row_ptr, W2->Ap.data(), (W2->m + 1) * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_W2_col_id, W2->Ai.data(), W2->nnz * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_W2_val, W2->Ax.data(), W2->nnz * sizeof(float), cudaMemcpyHostToDevice));
    ...
    SpMM<<<blocks, threads>>>(d_W1_row_ptr, d_W1_col_id, d_W1_val, d_input, d_H, hiddenSize, batchSize, inputSize);
    ...
    SpMM<<<blocks, threads>>>(d_W2_row_ptr, d_W2_col_id, d_W2_val, d_H, d_Z, outputSize, batchSize, hiddenSize);
    CUDA_CHECK(cudaGetLastError());
...
DenseMatrix *gpu_sparseNNSpmv(DenseMatrix *InData, CSR *W1, CSR *W2, 
    ...
    for (int i = 0; i < batchSize; i++) {
        ...
        int blocks = (hiddenSize + threads - 1) / threads;
        SpMV<<<blocks, threads>>>(d_W1_row_ptr, d_W1_col_id, d_W1_val, d_input, d_H, hiddenSize, inputSize);
        CUDA_CHECK(cudaGetLastError());
        ...
        blocks = (outputSize + threads - 1) / threads;
        SpMV<<<blocks, threads>>>(d_W2_row_ptr, d_W2_col_id, d_W2_val, d_H, d_Z, outputSize, hiddenSize);
        CUDA_CHECK(cudaGetLastError());
